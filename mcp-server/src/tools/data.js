...
export function registerDataTools(server, gw) {
  server.tool(
    'list_tables',
    'List all NocoDB tables in the ASuite workspace. Returns table IDs and titles.',
    {},
    async () => {
  ...
  server.tool(
    'describe_table',
    'Get the schema of a NocoDB table — column names, types, and constraints. Use this before query_rows to understand what columns exist.',
    {
  ...
  server.tool(
    'query_rows',
    'Query rows from a NocoDB table. Supports filtering with where clauses and sorting.',
    {
  ...
  server.tool(
    'insert_row',
    'Insert a new row into a NocoDB table. Pass column values as key-value pairs.',
    {
  ...
  server.tool(
    'update_row',
    'Update an existing row in a NocoDB table.',
    {
  ...
  server.tool(
    'delete_row',
    'Delete a row from a NocoDB table.',
    {
      table_id: z.string().describe('Table ID'),
